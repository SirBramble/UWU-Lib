...
inline uint8_t desc_hid_report[] = {
...
inline void init_sender(int bluetooth_timeout)
{
#if IS_MCU_VERSION != 0
    usb_hid.begin();
...
inline void send_kecycode_keyboard(keycode_node* node)
{
    if (!bluetooth_mode)
    {
#if IS_MCU_VERSION != 0
...
inline void send_keycode_node(keycode_node* node)
{
    if (node == nullptr)
        return;
...
inline void send_keycode_single(keycode_node* root)
{
    int free_pos = -1;
    int r_id = root->r_id;
...
inline void send_keycode(keycode_node* root, bool is_single)
{
    if (root == nullptr)
        return;
...
inline void clear_keycode(keycode_node* root, bool is_single)
{
    int r_id = root->r_id;
